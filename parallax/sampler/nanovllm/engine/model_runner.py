...
from nanovllm.m2n import M2NReceiver
...
class ModelRunner:
    ...
    def __init__(self, config: Config, rank: int, event: Event | list[Event]):
        ...
        self.weight_receiver = M2NReceiver(self.model, config, rank)
    ...
    def initialize_weight_transfer(self, unique_id: bytes) -> None:
        self.weight_receiver.connect(unique_id)
    ...
    def prepare_weight_update(
        ...
        self.weight_receiver.prepare(manifest)
        dist.barrier()
    ...
    def run(self, seqs: list[Sequence], is_prefill: bool) -> tuple[list[int], list[float]]:
        ...
        outputs = self.sampler(logits, temperatures) if self.rank == 0 else None
        reset_context()
        return tuple(output.tolist() for output in outputs) if self.rank == 0 else None
